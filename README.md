...
把 DeepSeek Harness（DSH，网页版 AI 助手）封装成一个独立的 Windows 桌面软件：
WebView2 内嵌窗口 + 内置 Node，双击就能用，适合完全不懂电脑的小白用户。
...
```
DeepSeek-Harness-Client.exe      主程序
Microsoft.Web.WebView2.*.dll    内嵌浏览器（WebView2）
WebView2Loader.dll               内嵌浏览器加载器
runtime\node\node.exe            内置独立 Node.js
config.txt                       配置（路径都在这里改）
源码\launcher.cs                 客户端源码（C#）
源码\build.bat                   一键编译脚本
```
...
Muxue-Dev（幻曦之殇） · GitHub: github.com/Muxue-Dev · B站: space.bilibili.com/110628804
